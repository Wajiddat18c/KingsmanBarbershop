...
const userIndexPage = fs.readFileSync(
  __dirname + "/../public/userlogin/user_index.html",
  "utf8"
);
...
const adminIndexPage = fs.readFileSync(
  __dirname + "/../public/adminlogin/admin_index.html",
  "utf8"
);
...
async function validate_user(req, res) {
  ...
  try {
    ...
    console.log(error);
    return res.redirect(
      "/signup/Ukendt fejl, prøv igen, eller kontakt admininistratoren."
    );
  }
}
    ...
    User.query()
      ...
      .then(async (foundUser) => {
        if (foundUser.length > 0) {
          return res.redirect(
            "/signup/E-mailen er allerede i brug, log ind eller gendan kodeordet."
          );
        } else {
          ...
          var mailOptions = {
            ...
            text: `Hej ${name},
                  ...
                  Her er en bekræftelse på din registering af bruger hos Kingsman barbershop
                  
                  Hav en god dag.`, // plaintext body
            html: `Hej ${name}, 
                      <br>
                      <br>Her er en bekræftelse på din registering af bruger hos Kingsman barbershop
                      <br>
...
router.get("/users", async (req, res) => {
  ...
  const user = await User.query().select("*").where("email", mail).limit(1);
...
router.get("/userAccount/delete/:email", async (req, res) => {
  if(req.session.isOn !== true)
        return res.redirect("/")
  await User.query().delete().where("email", "=", req.session.email);
  req.session.isOn == false;
